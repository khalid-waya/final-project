...
def normalize_features(stock_data, window_size=100):
    """
    Normalizes selected feature
    s using a rolling window approach.
...
feature_scaler.fit(stock_data[['MA for 50 days', 'RSI', 'Daily Return', 'MACD']])

# Create a separate scaler for the 'Close' price
close_scaler = MinMaxScaler()
close_scaler.fit(stock_data[['Close']])

# Normalize the features using the feature scaler
normalized_features = pd.DataFrame(feature_scaler.transform(stock_data[['MA for 50 days', 'RSI', 'Daily Return', 'MACD']]),
                                   columns=['MA for 50 days', 'RSI', 'Daily Return', 'MACD'],
                                   index=stock_data.index)

# Normalize the 'Close' price using the close scaler
normalized_close = pd.DataFrame(close_scaler.transform(stock_data[['Close']]),
                                columns=['Close'],
                                index=stock_data.index)

# Combine the normalized features and 'Close' price into a single DataFrame
normalized_df = pd.concat([normalized_close, normalized_features], axis=1)
X, Y = [], []
window_size = 100

# Create the input sequences and target values
for i in range(len(normalized_df) - window_size - 1):
    X.append(normalized_df.iloc[i:i+window_size][['Close', 'MA for 50 days', 'RSI', 'Daily Return', 'MACD']].values)
    Y.append(normalized_df.iloc[i + window_size]['Close'])
...
train_X = train_X.reshape(train_X.shape[0],100,5)
test_X = test_X.reshape(test_X.shape[0],100,5)
...
joblib.save(feature_scaler, "M:\ce301\gui\scaler.pkl")
...
rmse.update_state(y_test,predicted_prices)
...
denormalized_predicted_prices = close_scaler.inverse_transform(predicted_prices)

# Inverse transform the test prices using the close scaler
denormalized_test_prices = close_scaler.inverse_transform(y_test)
...
result_df = pd.DataFrame({
    ...
    'Predicted Price': denormalized_predicted_prices.flatten(),
    'Test Price': denormalized_test_prices.flatten()
...
plt.title('Apple Stock Price Prediction With 5 Features')
plt.xlabel('Date')
plt.ylabel('Price')
plt.legend()
plt.show()
...
plt.title('Training and Validation Loss  With 5 Features')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.show()
...
plt.title('Training and Validation Mean Squared Error  With 5 Features')
plt.xlabel('Epoch')
plt.ylabel('MSE')
plt.legend()
plt.show()
...
plt.title('Training and Validation Mean Absolute Error With 5 Features')
plt.xlabel('Epoch')
plt.ylabel('MAE')
plt.legend()
plt.show()